...
export default {
  setup() {
    ...
    function checkForbiddenVisibility(id) {
      ...
      return (allowAll.value && id != allowAllId.value) || allowAll.value == undefined
    }

    function hasChanged(id) {
      if (document.getElementById(`allowed${id}`).checked) return id in originalRoleAbilities.value ? false : true

      if (document.getElementById(`forbidden${id}`).checked)
        return id in originalForbiddenAbilities.value ? false : true

      if (!document.getElementById(`allowed${id}`).checked || !document.getElementById(`forbidden${id}`).checked)
        return id in originalRoleAbilities.value || id in originalForbiddenAbilities.value ? true : false
    }
    ...
    function customAllow(id) {
      if (document.getElementById(`allowed${id}`).checked) {
        if (id == allowAllId.value) {
          ...
          changed.value.splice(allowViewAllId.value, 1, hasChanged(allowViewAllId.value))
        ...
        allowViewAll.value = id == allowViewAllId.value ? undefined : allowViewAll.value
        ...
        .then(function (response) {
          ...
          if (typeof response.data.id === 'object') {
            for (let id in response.data.id) {
              changed.value.splice(response.data.id[id], 1, hasChanged(response.data.id[id]))
            }
          } else {
    ...
    function capabilityUpdate(id) {
      ...
      axios({
        ...
        .then(function (response) {
          if (response.data.capabilities[id].isCapable != capabilities[id].isCapable) {
            alert('Error')
    ...
    return {
      ...
      customAbilities,
      roleAbilities,
      originalRoleAbilities,
      originalForbiddenAbilities,
      roleForbiddenAbilities,
      modelAbilities,
      originalModelAbilities,
      permissions,
      capabilities,
      loadingSpinner,
      capabilities,
