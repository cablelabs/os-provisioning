...
function initSidebar() {
  ...
  props.netelements.forEach(n => {
    ...
    if (activeNetelement.value &&  clickedNetelement.value && n.id == activeNetelement.value && !isSearchMode.value) {
      loadCluster(n)
    }
  })
}
...
function handleMinify() {
  ...
  $(window).trigger('resize')
    if ($('table.datatable').length && ! store.minified) { // resize datatables after animation
...
function loadCluster(netelement) {
  ...
  .then((response) => {
    ...
    props.netelements[props.netelements.findIndex(n => n.id === netelement.id)].clusters = response.data
...
function favorNetelement(netelement) {
  ...
  axios({
    method: 'post',
    url: '/admin/Netelement/' + netelement.id + '/' + (props.favorites.includes(netelement.id) ? 'unfavorite' : 'favorite'),
    contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
  })
  .then(() => {
    ...
    if (props.favorites.includes(netelement.id)) {
      props.netelements.splice(props.netelements.findIndex(n => !initialNE.value && n.id === netelement.id), 1)
