...
@DivOpen(12)
	...
	<ul class="flex nav nav-pills nav-fill" id="SettingsTab">
		@foreach($moduleModels as $count => $model)
			<li class="nav-item">
				<a href="#settings-{{Str::slug($links[$count]['name'],'_')}}" data-toggle="pill" class="text-gray-400  hover:text-white"> {{ $links[$count]['name'] }} </a>
			</li>
		@endforeach
	</ul>
@DivClose()
