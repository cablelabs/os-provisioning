<div v-cloak id="sidebar-right" class="fixed h-screen right-0 z-[1010] flex text-gray-100">
    <div class="h-full bg-sidebar-dark w-64 transition-all duration-200 relative"
        ...
        <div class="absolute -left-5 bg-lime-nmsprime w-5 pt-2 space-y-4 h-full mt-12">
            <div class="hover:cursor-pointer" v-on:click="minifySidebarRight">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ease-in-out duration-300" :class="{ 'rotate-180': store.minifiedRight, 'rotate-0': !store.minifiedRight }" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                </svg>
            </div>
            <div v-if="!store.minifiedRight" class="hover:cursor-pointer" v-on:click="pinSidebarRight">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ease-in-out duration-300" :class="{ 'block rotate-90': pinned, 'block -rotate-90 hover:rotate-0': !pinned }" fill="currentColor" viewBox="0 0 384 512"
