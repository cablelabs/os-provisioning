...
@section('content_left')
    ...
    @if (session('pdf_creation_test'))
        ...
        @if (session('pdf_creation_test.pdflatex_output'))
        ...
        @if (session('pdf_creation_test.other_files'))
            ...
            @foreach (session('pdf_creation_test.other_files') as $filename => $content)
                ...
                <div class='collapse' id='collapse_{{ $filename }}'>
                    <pre style='border-left: solid #aaa 2px; padding: 20px; margin-left: 20px; background-color: #eee;'>{!! $content !!}</pre>
                </div>

            @endforeach
        @endif

        @if (session('pdf_creation_test.usable_placeholders'))
