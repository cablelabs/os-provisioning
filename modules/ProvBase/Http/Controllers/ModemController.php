...
class ModemController extends \BaseController
{
    ...
    public function cpeAnalysis($id)
    {
        ...
        $str = getLogEntries('/var/log/messages', $modem_mac, '| grep CPE | tail -n 1 | tac');
        ...
        if ($str == []) {
            ...
            $mac = trim($mac);
            $mac_bug = true;
            $str = getLogEntries('/var/log/messages', $mac, '| grep CPE | tail -n 1 | tac');
        ...
        if (isset($cpeMac[0][0])) {
            $cpeMac = $cpeMac[0][0];
            $log = getLogEntries('/var/log/messages', $cpeMac, '| tail -n 20 | tac');
